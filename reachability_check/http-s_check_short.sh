...
check_url() {
  ...
  tls_info=$(echo "$output" | grep -oP 'SSL connection using \K.*')
  ...
  # Output
  echo -ne "[${color}${BOLD}${status_message}${NC}] $http_code  "
  echo -ne "${BOLD}"$url"${NC}  "
  echo -ne "$resolved_ip  "
  echo -ne "$cert_status Cert  "
  #echo -ne "$url_effective"
}

extract_host() {
    local input="$1"
    input="${input#*://}"     # remove scheme
    echo "${input%%[:/]*}"    # extract up to : or / (host only)
}
